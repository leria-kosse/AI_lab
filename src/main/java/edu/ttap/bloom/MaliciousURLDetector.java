...
public class MaliciousURLDetector {
     ...
     */
    public static BloomFilter<String> makeURLFilter(String filename, int bitSetSize, int numHashFunctions) 
    throws IOException {
        ...
        for (String line : lines) {
            if (line.equals("url,type")) {continue;}
            ...
            if (parts.length >= 2) {
                ...
                if (!type.equals("benign")) {filter.add(url);}
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        if (args.length != 2) {
            System.out.println("Usage: mvn compile exec:java -Dexec.args=\"<bitset size> <num hash functions>\"");
            return;
        }
